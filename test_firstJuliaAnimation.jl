...
anim = animation.FuncAnimation(fig, animate, init_func=initPlot,frames=oldTraj.oldNIter[j]+start_offset-1, interval=1,repeat=false)

# to save video: you need encoder and writer, for ubuntu it should work if you do: sudo apt-get install ffmpeg
FFwriter = animation.FFMpegWriter( fps=8, extra_args=["-vcodec", "libx264"])
