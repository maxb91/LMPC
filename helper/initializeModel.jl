type initPathFollowingModel
    ...
    obstacle::classes.Obstacle, n_oldTraj::Int64)
        ...
        # Derivative cost
        # ---------------------------------
        @NLexpression(mdl, derivCost, sum(QderivZ[j] * sum((z_Ol[i,j] - z_Ol[i + 1,j]) ^ 2 for i = 1:N) for j = 1:6) +  #(z0[j]-z_Ol[1,j])^2
...
type initLearningModel
    ...
    obstacle::classes.Obstacle, n_oldTraj::Int64)
        ...
        z_Init          = zeros(6)
        z_Init[1]       = 0.6
        z_Ol_Init       = zeros(N+1,6)
        z_Ol_Init[:,1]  = v_ref*ones(N+1)
        ...
        z_Ol   = @variable( mdl, [i =1:(N+1),j = 1:6], lowerbound = z_lb[i,j], upperbound = z_ub[i,j], start =z_Ol_Init[i,j])      # z = s, ey, epsi, v
       ...
       #basic idea:
        ...
        sum(Q_obstacleNumer*1/(0.01+(Q_obstacle* (( (z_Ol[i,6]-sCoord_obst[i,1])/rs )^2 + ( (z_Ol[i,5]-sCoord_obst[i,2])/ry) ^2 - 1))^4)+
        Q_obstacleNumer*3/(0.01+0.6*(((z_Ol[i,6]-sCoord_obst[i,1])/rs)^2+((z_Ol[i,5]-sCoord_obst[i,2])/ry)^2)) for i=1:N+1))
