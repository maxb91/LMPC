type initLearningModel
    ...
    obstacle::classes.Obstacle, n_oldTraj::Int64)
        ...
        u_Ref           = zeros(N,2)
        z_Ol_Init = zeros(N+1,4)
        z_Ol_Init[:,4] = ones(N+1)
        v_max = 2.0
        #########################################################
        ...
        z_Ol   = @variable( mdl, [i =1:(N+1),j = 1:4], lowerbound = z_lb[i,j], upperbound = z_ub[i,j], start =z_Ol_Init[i,j])      # z = s, ey, epsi, v
       ...
       #1/t
        ...
        @NLparameter(mdl, coeffTermConst[i=1:order+1,k=1:n_oldTraj,j=1:3] == coeffTermConst[i,k,j])
