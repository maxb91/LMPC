function saveOldTraj(oldTraj::OldTrajectory,zCurr::Array{Float64},uCurr::Array{Float64},lapStatus::LapStatus,buffersize::Int64,dt::Float64)
                ...
                zCurr_export    = cat(1,zCurr[1:i-1,:], [ones(buffersize-i+1,1)*zCurr[i-1,1:5] zCurr[i-1,6]+collect(1:buffersize-i+1)*dt*zCurr[i-1,1]])
...
function InitializeModel(m::MpcModel,mpcParams::MpcParams,modelParams::ModelParams,trackCoeff::TrackCoeff,z_Init::Array{Float64,1})
    ...
    @NLparameter(m.mdl, m.c_Vx[i=1:3] == 0)
    @NLparameter(m.mdl, m.c_Vy[i=1:4] == 0)
    @NLparameter(m.mdl, m.c_Psi[i=1:3] == 0)
    ...
    # System dynamics
    for i=1:N
        # @NLconstraint(m.mdl, m.z_Ol[i+1,1]  == m.z_Ol[i,1] + dt*(m.u_Ol[i,1] + m.z_Ol[i,2]*m.z_Ol[i,3] - 0.63*abs(m.z_Ol[i,4]) * m.z_Ol[i,4]))  # xDot
        # @NLconstraint(m.mdl, m.z_Ol[i+1,2]  == m.z_Ol[i,2] + dt*(2/m*(m.FyF*cos(u_Ol[i,2])+m.FyR) - z_Ol[i,3]*z_Ol[i,1]))                       # yDot
        # @NLconstraint(m.mdl, m.z_Ol[i+1,3]  == m.z_Ol[i,3] + dt*(2/I_z*(L_a*m.FyF-L_b*m.FyR)))                                                  # psiDot
        
        #@NLconstraint(m.mdl, m.z_Ol[i+1,1]  == m.z_Ol[i,1] + dt*(m.c_Vx[3]*m.z_Ol[i,1] + m.c_Vx[1]*m.z_Ol[i,2] + m.c_Vx[2]*m.z_Ol[i,3] + m.u_Ol[i,1]))  # xDot
        ...
        @NLconstraint(m.mdl, m.z_Ol[i+1,6]  == m.z_Ol[i,6] + dt*m.dsdt[i]  )                                                                    # s
    end
end


function InitializeModel_pathFollow(m::MpcModel,mpcParams::MpcParams,modelParams::ModelParams,trackCoeff::TrackCoeff,z_Init::Array{Float64,1})
    dt   = modelParams.dt
    L_a  = modelParams.l_A
    L_b  = modelParams.l_B
    c0   = modelParams.c0
...
function InitializeParameters(mpcParams::MpcParams,mpcParams_pF::MpcParams,trackCoeff::TrackCoeff,modelParams::ModelParams,
    ...
    mpcCoeff.coeffConst         = zeros(mpcCoeff.order+1,2,5) # nz-1 because no coeff for s
