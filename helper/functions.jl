...
function InitializeModel(m::classes.MpcModel,mpcParams::classes.MpcParams,modelParams::classes.ModelParams,trackCoeff::classes.TrackCoeff,z_Init::Array{Float64,1}, obstacle::classes.Obstacle)
   ...
   #1/m.t
    ...
    @NLexpression(m.mdl, constZTerm, (sum{Q_term[j]*( m.lambda[1]*sum{coeffTermConst[i,1,j]*m.z_Ol[N+1,1]^(order+1-i),i=1:order+1}+
                                                        m.lambda[2]*sum{coeffTermConst[i,2,j]*m.z_Ol[N+1,1]^(order+1-i),i=1:order+1}+
                                                        m.lambda[3]*sum{coeffTermConst[i,3,j]*m.z_Ol[N+1,1]^(order+1-i),i=1:order+1}+
                                                        m.lambda[4]*sum{coeffTermConst[i,4,j]*m.z_Ol[N+1,1]^(order+1-i),i=1:order+1}+
                                                        m.lambda[5]*sum{coeffTermConst[i,5,j]*m.z_Ol[N+1,1]^(order+1-i),i=1:order+1}-
   ...
   #basic idea
    ...
    @NLexpression(m.mdl, costZTerm, Q_cost*(  m.lambda[1]*sum{coeffTermCost[i,1]*m.z_Ol[N+1,1]^(order+1-i),i=1:order+1}+
                                                m.lambda[2]*sum{coeffTermCost[i,2]*m.z_Ol[N+1,1]^(order+1-i),i=1:order+1}+
                                                m.lambda[3]*sum{coeffTermCost[i,3]*m.z_Ol[N+1,1]^(order+1-i),i=1:order+1}+
                                                m.lambda[4]*sum{coeffTermCost[i,4]*m.z_Ol[N+1,1]^(order+1-i),i=1:order+1}+
                                                m.lambda[5]*sum{coeffTermCost[i,5]*m.z_Ol[N+1,1]^(order+1-i),i=1:order+1}))
    #basic idea    
    ...
    return costPath, costZ, costZTerm,constZTerm,derivCost,controlCost,laneCost,costObstacle
end
